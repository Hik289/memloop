...
# V6: Dual-Memory prompt — controlled by MEMLOOP_V6_DUAL_MEM=1
...
# V6.1: Scan-first + citation completeness check — controlled by MEMLOOP_V61=1
...
# V7: Entity-matching + exhaustive scan — controlled by MEMLOOP_V7_ENTITY=1
...
def _node_text_for_answer(n, is_top_detail: bool = False) -> str:
    """Return node text for answer context based on MEMLOOP_ANSWER_MODE.
    ...
    """
    mode = _os_ansmode.environ.get("MEMLOOP_ANSWER_MODE", "detailed_truncated")
    ...
    if mode == "mixed":
        ...
        _brief_mode = _os_ansmode.environ.get("MEMLOOP_BRIEF_MODE", "distilled")
...
def format_context_block(items: List[Tuple[str, str, str]]) -> str:
    ...
    if _os_fmt.environ.get("MEMLOOP_V6_DUAL_MEM", "0") == "1":
...
def _format_dual_smart_context(
    ...
    """
    import os as _os_ds
    top_k = int(_os_ds.environ.get("MEMLOOP_DUAL_SMART_K", str(top_detailed_k)))
...
def get_index(ctx: RunnerCtx, level: str) -> CosineIndex:
    ...
    if not ids:
        ...
        return ctx.indexes[level]
    # MEMLOOP_SKIP_L0_EMBED=1: skip ONLY L0 embedding (avoids 207K-node 429); L1/L2 nav still works
    import os as _os_skip
    if level == "L0" and _os_skip.environ.get("MEMLOOP_SKIP_L0_EMBED", "0") == "1":
...
def run_b_hier_bfs(ctx: RunnerCtx, qid: str, query: str) -> QueryRecord:
    ...
    for nid, _ in l0_hits[: ctx.top_k_distilled]:
        ...
        seen.add(nid)
        n = ctx.hierarchy[nid]
        _ans_mode_cur = _os_ansmode.environ.get("MEMLOOP_ANSWER_MODE", "")
        _n_l0_so_far = sum(1 for _x in ctx_items if _x[1] == "L0")
        _top_k_det = int(_os_ansmode.environ.get("MEMLOOP_MIXED_TOP_K", "5"))
...
def final_l0_retrieval(ctx: RunnerCtx, q_vec, seed_ids: Sequence[str],
    ...
    if _os.environ.get("MEMLOOP_L0_RETRIEVAL", "") == "bm25":
        ...
        if _os.environ.get("MEMLOOP_CLUSTER_FILTER", "0") == "1" and seed_ids:
            ...
            _wide_k = int(_os.environ.get("MEMLOOP_BM25_WIDE_K", "50"))
        ...
        if _os.environ.get("MEMLOOP_QUERY_EXPAND", "0") == "1" and seed_ids:
        ...
        if _os.environ.get("MEMLOOP_RRF", "0") == "1":
            _rrf_wide = int(_os.environ.get("MEMLOOP_RRF_WIDE_K", "50"))
        ...
        if _os.environ.get("MEMLOOP_TE3_RRF", "0") == "1":
            _load_te3_index_once()
            if _TE3_ARR is not None and len(_TE3_ARR) > 0:
                _rrf_wide_c4 = int(_os.environ.get("MEMLOOP_RRF_WIDE_K", "50"))
...
def run_llm_nav(ctx: RunnerCtx, qid: str, query: str, query_idx: int,
    ...
    for step in range(ctx.max_steps):
        ...
        if action == "DESCEND":
            ...
            import os as _os_mp
            if _os_mp.environ.get("MEMLOOP_MULTIPATH_TOP2", "0") == "1":
    ...
    if final_action == "ANSWER":
        ...
        if (use_promotion and ctx.promotion is not None
            ...
            if _bm25_cand_pool:
                ...
                import os as _os_c3
                if _os_c3.environ.get("MEMLOOP_PROMO_RETRIEVAL", "0") == "1":
                    _c3_extra_ids: list = []
        ...
        _use_dual_smart = _os_dsm.environ.get("MEMLOOP_ANSWER_MODE", "") == "dual_smart"
        if _use_dual_smart:
        ...
        _use_v6 = _os_v6dm.environ.get("MEMLOOP_V6_DUAL_MEM", "0") == "1"
        _use_v7 = _os_v6dm.environ.get("MEMLOOP_V7_ENTITY", "0") == "1"
        _use_v61 = _os_v6dm.environ.get("MEMLOOP_V61", "0") == "1"
        _ans_mode_v8 = _os_v6dm.environ.get("MEMLOOP_ANSWER_MODE", "")
